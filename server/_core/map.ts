/**
 * Google Maps API integration via the configured OpenAI-compatible gateway (maps proxy).
 *
 * Main function: makeRequest<T>(endpoint, params) — authenticated requests to Google Maps APIs.
...
function getMapsConfig(): MapsConfig {
  ...
  if (!baseUrl || !apiKey) {
    throw new Error(
      "Google Maps proxy credentials missing: set LLM_API_BASE_URL (or OPENAI_BASE_URL) and LLM_API_KEY (or OPENAI_API_KEY)"
    );
  }
